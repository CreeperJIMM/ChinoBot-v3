        ...
        Mongo.loadGuild(clientDB, member.guild.id).then((user) => {
            if (user === false) {
                ...
                if (user.join != null) {
                    ...
                    try {
                        client.channels.cache.get(user.join);
                    } catch (error) {
                        return;
                    }
                    ...
                    if (!send1) return;
                    try {
                        return client.channels.cache.get(user.join).send(send1);
                    } catch (error) {
                        return;
                    }
                }
        ...
        Mongo.loadGuild(clientDB, member.guild.id).then((user) => {
            if (user === false) {
                ...
                if (user.leave != null) {
                    ...
                    if (!send1) return;
                    try {
                        return client.channels.cache.get(Leave).send(send1);
                    } catch (error) {
                        return;
                    }
                }
